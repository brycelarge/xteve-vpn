...
if [[ -z "${1}" ]]; then
    echo "[OpenVPN config clean] no parameter supplied for file to convert" | ts '%Y-%m-%d %H:%M:%S'
    exit 1
fi

if [[ ! -f "${1}" ]]; then
    echo "[OpenVPN config clean] file '${1}' does not exist" | ts '%Y-%m-%d %H:%M:%S'
    exit 1
fi
...
if ! cat "${STDOUT_FILE}" | grep -q 'Converted 0'; then
    echo "[OpenVPN config clean] line ending conversion required, moving '${TEMP_FILE}' to '${SOURCE_FILE}'"
